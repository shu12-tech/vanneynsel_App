...
export interface Message {
  ...
  >(
    ...
    async (_, { getState, rejectWithValue }) => {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}${GET_CHAT_HISTORY}/${auth.user.username}/${session.currentSession.session_id}`, {
          method: 'GET',
          headers: {
  ...
  const promptSlice = createSlice({
    ...
    extraReducers: (builder) => {
      builder
        ...
        .addCase(getMessages.rejected, (state, action: PayloadAction<string | undefined>) => {
          state.loading = false;
  ...
  export const { setPrompt, addMessage, updateLastMessage, clearMessages, setAsk } = promptSlice.actions;
